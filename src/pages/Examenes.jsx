...
export default function Examenes({ navigate, initialFilter = {} }) {
  const { user } = useAuth();

  const [search, setSearch]               = useState("");
  const [tipoFilter, setTipoFilter]       = useState(initialFilter.tipo ?? "all");
  ...
  const filtered = useMemo(() => {
    let list = examenes.filter(e => {
      ...
      const matchCurso    = cursoFilter === "all" || String(e.cursoId) === cursoFilter;
      ...
      const matchDocente  = docenteFilter === "all" || String(e.docenteId) === docenteFilter;

      return matchSearch && matchTipo && matchCurso && matchFacultad && matchCarrera
        && matchCiclo && matchSem && matchResuelto && matchGratis && matchDocente;
    });
    ...
    return list;
  }, [search, tipoFilter, cursoFilter, facultadFilter, carreraFilter, cicloFilter, semestreFilter, soloResueltos, soloGratis, sortBy, docenteFilter]);
  ...
  const hasFilter = search || tipoFilter !== "all" || facultadFilter !== "all" || carreraFilter !== "all"
    || cursoFilter !== "all" || cicloFilter !== "all" || semestreFilter !== "all"
    || soloResueltos || soloGratis || docenteFilter !== "all";

  const reset = useCallback(() => {
    setSearch(""); setTipoFilter("all"); setFacultad("all"); setCarrera("all");
    setCurso("all"); setCiclo("all"); setSemestre("all");
    setSoloResueltos(false); setSoloGratis(false); setDocente("all");
  }, []);
    ...
    <>
      ...
      <div className="ex-page page-container">
        ...
        {(cursoFilter !== "all" || carreraFilter !== "all") && (() => {
          ...
          return (
            <div className="ex-active-filter-banner">
              <span>🔍 Filtrando por:</span>
              {cursoActivo && <strong>📚 {cursoActivo.nombre}</strong>}
              {carreraActiva && <strong>🎓 {carreraActiva.nombre}</strong>}
              <button className="ex-clear-filter-btn" onClick={reset}>✕ Quitar filtro</button>
            </div>
          );
        })()}
                  ...
                  <div className="ex-row" key={ex.id} onClick={() => setModalExamen(ex)}>
                    <div className="ex-col-main">
                      ...
                      <div className="ex-row-meta">
                        ...
                        {facultad && <span className="ex-row-fac" style={{ color: facultad.color }}>{facultad.icono}</span>}
